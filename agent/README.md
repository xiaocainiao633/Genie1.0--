...
- **自动脚本生成**：根据自然语言自动组合 AutoGo API（motion/uiacc/opencv/ppocr/yolo…）
...
```bash
# 1. 初始化知识库
go run main.go -init -kb ./test.db

# 2. 启动对话系统
go run main.go -init -kb ./test.db -use-llm

# 3. 单次查询
go run main.go -kb ./test.db -workspace ./workspace \
